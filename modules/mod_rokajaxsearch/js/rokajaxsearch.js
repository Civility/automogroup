...
var L;if(this.options.showcategory){var J=new Element("span",{"class":"small"}).set("html",A.getChildren().getLast().get("text")).inject(H,"after");L=new Element("i").inject(J,"after");
}if(this.options.showreadmore){H=new Element("a",{"class":"clr"}).set("href",F).set("html",this.options.readmore).inject(I,"after");if(this.options.showdescription){L=new Element("i").inject(I,"after");
...
if(!g.length){var a=new Element("div",{"class":this.rows[0]});
	u=new Element("h3").set("html",this.options.noresults).inject(a);var t=new Element("a").set("href",this.options.advsearchlink).inject(u,"after");
...
Q=new Element("i").inject(N,"after");}}if(this.options.showreadmore){K=new Element("a",{"class":"clr"}).set("href",G).set("html",this.options.readmore).inject(J,"after");
if(this.type!="local"){K.set("target","_blank");}if(this.options.showdescription){Q=new Element("i").inject(J,"after");}}var E=new Element("div",{"class":"roksearch_result_wrapper1 png"}).inject(w);
...
}for(j=0,l=p;j<l;j++){if(c[A*p+j]){c[A*p+j].inject(this.page[A]);}}}.bind(this));

if(this.type!="local"){var h=this.page[0].getSize();this.page[0].setStyle("position","relative");
...
a.inject(q);}else{
	...
	if(this.options.includelink){var x=document.getElements("#rokajaxsearch input[name=limit]")[0];
	this.bottombar=new Element("div",{"class":"roksearch_row_btm png"});
var b=new Element("a",{"class":"viewall"}).set("href","#").inject(this.bottombar);z=new Element("span").set("html",this.options.viewall).inject(b);
	if(this.type!="local"){b.setProperties({href:this.google.cursor.moreResultsUrl,target:"_blank"});
