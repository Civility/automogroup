...
require_once(dirname(__FILE__).'/library/parameter.php');
require_once(dirname(__FILE__).'/library/flatArray.php');
jimport('joomla.filesystem.folder');

if(JRequest::getVar('option') == 'com_joomfish') return;
...
class JElementTheme extends JOfflajnFakeElementBase{
  ...
  function universalFetchElement($name, $value, &$node){
    ...
    $document->addScript(JURI::base().'../modules/'.$this->_moduleName.'/params/dojo.xd.js');
  ...
  function generateThemeSelector($name, $value){
    ...
    if ( is_array($themes) ){
        ...
        $this->params->theme = $theme;
    		$this->params->addElementPath( JPATH_ROOT . str_replace('/', DIRECTORY_SEPARATOR, '/modules/'.$this->_moduleName.'/params') );
        ...
        for($x = 0; count(@$_xml['_default']->_children) > $x; $x++){
          ...
          if(isset($node->_attributes['directory'])){
            $node->_attributes['directory'] = str_replace('/', DIRECTORY_SEPARATOR, '/modules/'.$this->_moduleName.'/themes/'.$theme.'/'.$node->_attributes['directory']);
          }
        }
